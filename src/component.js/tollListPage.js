...
function TollListPage(props) {
  console.log("poppps", props.tollList);
  const deleteToll=(id)=>{
  ...
  return (
    <div style={{ marginTop: "20px" }}>
      <table style={{ width: "100%" }}>
        ...
        {props.tollList.filter(e=>(e.toll_name?.toUpperCase()?.includes(props.searchToll)|| props.searchToll ==undefined)).map((toll, key) => {
          return (
            <tr onMouseOut={()=>document.getElementById(`delete_${key}`).style.opacity =0} onMouseMove={()=>document.getElementById(`delete_${key}`).style.opacity =1} key={toll.toll_id}>
              <td
                style={{
                  width: "40%",
                  textAlign: "left",
